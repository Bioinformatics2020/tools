...
void SChunkReference::OnNodeDoubleClicked(UEdGraphNode *Node) const
{
    ...
    //显示间接引用的资产
    AddExternalDependenciesItem(ExternalDependencies);

    //显示主资产直接包含的资产
    AddInternalDependenciesItem(InternalDependencies);
}

void SChunkReference::AddExternalDependenciesItem(const TArray<UEdGraphNode_ChunkReference::FAssetDependenciesInfo> &ExternalDependencies) const
{
    ...
    for ( int i = 0; i < ExternalDependencies.Num(); i++ )
    {
        ...
        if ( ExternalDependencies[i].AssetName.Find("/Game") != 0 )
        {
            continue;
        }
        ...
        switch ( ExternalDependencies[i].ChunkType )
        {
            ...
            case UEdGraphNode_ChunkReference::FAssetDependenciesInfo::ParentChunk:
                TextStyle.ColorAndOpacity = FLinearColor(0.0f, 1.0f, 0.5f);
            break;
...
void SChunkReference::AddInternalDependenciesItem(const TArray<FString> &InternalDependencies) const
{
