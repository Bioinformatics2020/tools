...
#include "TcpSocketConnect.generated.h"
...
class NIC_API ATcpSocketConnect : public AActor
{
	GENERATED_BODY()
...
public:	
	...
	our ATcpSocketConnect actor will be destroyed, an then the thread will send a message through AsyncTask to call ExecuteOnConnected, 
	...
	*/
	...
	void ExecuteOnConnected(int32 WorkerId, TWeakObjectPtr<ATcpSocketConnect> thisObj);

	//UFUNCTION(Category = "Socket")
	void ExecuteOnDisconnected(int32 WorkerId, TWeakObjectPtr<ATcpSocketConnect> thisObj);

	//UFUNCTION(Category = "Socket")
	void ExecuteOnMessageReceived(int32 ConnectionId, TWeakObjectPtr<ATcpSocketConnect> thisObj);
...
private:
	...
	FString ipAddress;
	int port;
	TWeakObjectPtr<ATcpSocketConnect> ThreadSpawnerActor;
	int32 id;
...
public:
	...
	FTcpSocketWorker(FString inIp, const int32 inPort, TWeakObjectPtr<ATcpSocketConnect> InOwner, int32 inId, int32 inRecvBufferSize, int32 inSendBufferSize, float inTimeBetweenTicks);
