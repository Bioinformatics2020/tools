...
// Sets default values
ATcpSocketConnect::ATcpSocketConnect()
{
...
void ATcpSocketConnect::BeginPlay()
{
	Super::BeginPlay();	
}

void ATcpSocketConnect::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
// Called every frame
void ATcpSocketConnect::Tick(float DeltaTime)
{
	Super::Tick(DeltaTime);
}

void ATcpSocketConnect::Connect(const FString& ipAddress, int32 port, const FTcpSocketDisconnectDelegate& OnDisconnected, const FTcpSocketConnectDelegate& OnConnected,
	const FTcpSocketReceivedMessageDelegate& OnMessageReceived, int32& ConnectionId)
{
	...
	//UE_LOG(LogTemp, Log, TEXT("Tcp Socket: connected IP:%s Port:%d"),*ipAddress,port);
	TWeakObjectPtr<ATcpSocketConnect> thisWeakObjPtr = TWeakObjectPtr<ATcpSocketConnect>(this);
...
void ATcpSocketConnect::Disconnect(int32 ConnectionId)
{	
...
bool ATcpSocketConnect::SendData(int32 ConnectionId /*= 0*/, TArray<uint8> DataToSend)
{
...
void ATcpSocketConnect::ExecuteOnMessageReceived(int32 ConnectionId, TWeakObjectPtr<ATcpSocketConnect> thisObj)
{
...
TArray<uint8> ATcpSocketConnect::Concat_BytesBytes(TArray<uint8> A, TArray<uint8> B)
{
...
TArray<uint8> ATcpSocketConnect::Conv_IntToBytes(int32 InInt)
{
	TArray<uint8> result;
	for (int i = 0; i < 4; i++)
	{
...
TArray<uint8> ATcpSocketConnect::Conv_StringToBytes(const FString& InStr)
{
...
TArray<uint8> ATcpSocketConnect::Conv_FloatToBytes(float InFloat)
{
	TArray<uint8> result;
...
TArray<uint8> ATcpSocketConnect::Conv_ByteToBytes(uint8 InByte)
{
	TArray<uint8> result{ InByte };
	return result;
}

int32 ATcpSocketConnect::Message_ReadInt(TArray<uint8>& Message)
{
	if (Message.Num() < 4)
	{
...
uint8 ATcpSocketConnect::Message_ReadByte(TArray<uint8>& Message)
{
	if (Message.Num() < 1)
	{
...
bool ATcpSocketConnect::Message_ReadBytes(int32 NumBytes, TArray<uint8>& Message, TArray<uint8>& returnArray)
{
...
float ATcpSocketConnect::Message_ReadFloat(TArray<uint8>& Message)
{
	if (Message.Num() < 4)
	{
...
FString ATcpSocketConnect::Message_ReadString(TArray<uint8>& Message, int32 BytesLength)
{
	if (BytesLength <= 0)
	{
		if (BytesLength < 0)
...
bool ATcpSocketConnect::isConnected(int32 ConnectionId)
{
	if (TcpWorkers.Contains(ConnectionId))
...
void ATcpSocketConnect::PrintToConsole(FString Str, bool Error)
{
...
void ATcpSocketConnect::ExecuteOnConnected(int32 WorkerId, TWeakObjectPtr<ATcpSocketConnect> thisObj)
{
	if (!thisObj.IsValid())
		return;
...
void ATcpSocketConnect::ExecuteOnDisconnected(int32 WorkerId, TWeakObjectPtr<ATcpSocketConnect> thisObj)
{
	if (!thisObj.IsValid())
		return;
...
FTcpSocketWorker::FTcpSocketWorker(FString inIp, const int32 inPort, TWeakObjectPtr<ATcpSocketConnect> InOwner, int32 inId, int32 inRecvBufferSize, int32 inSendBufferSize, float inTimeBetweenTicks)
	: ipAddress(inIp)
	, port(inPort)
...
{
	AsyncTask(ENamedThreads::GameThread, []() {	ATcpSocketConnect::PrintToConsole("Tcp socket thread was destroyed.", false); });
	Stop();
	if (Thread)
	{
...
{
	AsyncTask(ENamedThreads::GameThread, []() {	ATcpSocketConnect::PrintToConsole("Starting Tcp socket thread.", false); });
	...
	while (bRun)
	{
		...
		if (!bConnected)
		{
			...
			{
				AsyncTask(ENamedThreads::GameThread, []() { ATcpSocketConnect::PrintToConsole(FString::Printf(TEXT("Couldn't connect to server. TcpSocketConnection.cpp: line %d"), __LINE__), true); });
				bRun = false;				
			}
			continue;
		}
		...
		if (!Socket)
		{
			AsyncTask(ENamedThreads::GameThread, []() { ATcpSocketConnect::PrintToConsole(FString::Printf(TEXT("Socket is null. TcpSocketConnection.cpp: line %d"), __LINE__), true); });
			bRun = false;
			continue;
		}
		...
		while (bRun)
		{
			...
			AsyncTask(ENamedThreads::GameThread, []() { ATcpSocketConnect::PrintToConsole("Pending data", false); });
			...
			if (!Socket->Recv(receivedData.GetData() + BytesReadTotal, PendingDataSize, BytesRead))
			{
				...
				AsyncTask(ENamedThreads::GameThread, []() {
					ATcpSocketConnect::PrintToConsole(FString::Printf(TEXT("In progress read failed. TcpSocketConnection.cpp: line %d"), __LINE__), true);
				});
				break;
			}
		...
		if (timeToSleep > 0.f)
		{
			//AsyncTask(ENamedThreads::GameThread, [timeToSleep]() { ATcpSocketConnect::PrintToConsole(FString::Printf(TEXT("Sleeping: %f seconds"), timeToSleep), false); });
